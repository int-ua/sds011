...
# note: we are using sqlite for convenience
# as most people don't run a full scale sql server
...
class databasehandler():
    ...
    def init_db(self):
        ...
        if measurmenttable not in tables:
            c.execute(
                "create table {0}"
                "(timestamp datetime, pm2_5 float, pm10 float, devid integer)"
                .format(measurmenttable))
        conn.commit()
        conn.close()
    '''
    ...
    def add_measurement(self, measurement):
        ...
        if measurmenttable not in tables:
            c.execute(
                ...
                "(timestamp datetime, pm2_5 float, pm10 float, devid integer)"
                .format(measurmenttable))
        linedata = [measurement.get(x) for x in
                    ["timestamp", "pm2.5", "pm10", "devid"]]
        c.execute(
            "insert into {0} (timestamp,pm2_5,pm10,devid) values (?,?,?,?)"
            .format(measurmenttable),
            linedata)
        conn.commit()
        conn.close()
